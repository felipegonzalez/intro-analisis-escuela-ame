...
$$\min_{\beta_0,\beta_1,\ldots, \beta_p} \sum_{i=1}^n (y_i - f(x_i))$$

donde $x_i = (x_i^(1), \ldots, x_i^(p)$ son las variables de entrada para cada caso de entrenamiento.

Esto parece ser un poco contrario a lo que discutimos arriba: intentamos minimizar el error de entrenamiento,
cuando realmente quisiéramos tener error de predicción bajo. Los mismo podemos decir acerca
de k vecinos más cercanos. Cuando tenemos modelos chicos (pocas variables, muchos vecinos más cercanos) y muchos datos
(n grande), no es un problema muy grande. En parte lo es, y veremos más adelante cómo mitigar este
problema.
...
x_0 <- 7
```
En nuestro ejemplo anterior, consideremos qué pasa cuando hacemos predicciones en x = `r x`. Usaremos
