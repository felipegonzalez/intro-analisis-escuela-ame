...
recientemente y registramos su precio de venta. Comparamos el precio de venta
...
se usaba experiencia del área y reglas simples que un valuador podría usar.

En análisis predictivo, la idea es aprender de los datos *etiquetados* (donde conocemos la variable respuesta $y$) buenas reglas predictivas con
métodos computacionales, con el objetivo específico de que el error predictivo sea bajo.

Así es que emepezamos tomando unos **datos de entrenamiento**

$$({x}^{(1)}, {y}^{(1)}), ({x}^{(1)}, {y}^{(1)}), \ldots, ({x}^{(m)}, {y}^{(m)}),$$
Y queremos tener un método para construír $f$ a partir de los datos

$$({x}^{(1)}, {y}^{(1)}), ({x}^{(1)}, {y}^{(1)}), \ldots, ({x}^{(m)}, {y}^{(m)}) \to f$$
...
Queremos construir un $\mathcal{L} \to f $ (sólo depende de datos de entrenamiento) tal que el promedio
...
- ¿Por qué los errores de predicción pueden ser pobre?
...
¿Cómo explicamos los resultados que obtuvimos en el ejercicio anterior?
...
- Intuitivamente, es un método que dependen muy fuertemente de los datos. Si variamos un poco los datos de entrenamiento, las predicciones pueden cambiar mucho.
...
- Esto quiere decir que no encontró patrones de variación que **sí** generalizan a la muestra de prueba. **Subajustó** los datos de entrenamiento.
- También podemos decir que este modelo no pude aprender patrones estables
...
- Este modelo no pudo memorizar los datos tan bien, pero aprendió mejor
...
La función $f_0$ es modela la dependencia "verdadera" de $Y$ de los datos de entrada $X$
...
- $f(x) - \hat{f}(x) $ puede ser grande, es decir, nuestra estimación de $f_0$ es mala. A su vez,
hay dos razones por las que puede ocurrir esto:

- $f(x)$ tiene poca variabilidad y está consistemente lejos de la verdadera $f$, independientemente de la muestra
...
podemos demostrar que el error de predicción (con error cuadrático) se descompone en:
...
que nos servirá para ilustrar los primeros principio. 

Una primera idea de un predictor simple es $f$ constante
...
## k-vecinos más cercanos en caso simple {-}
...
En este caso, usamos 27-vecinos más cercanos:
...
Estos dos últimos modelos se desempeñan mal con respecto al que construimos con $k=25$. Podemos
...
minimizar, en lugar de la fórmula de arriba, 
...
$${f}(x ) = \hat{b}_0 +\ {\hat{b}_1} x_1 + \cdots + \hat{b} x_p$$
- La idea de usar $\lambda$ es que nos impida "sobreajustar" los datos, al poner una
...
valores muy positivos o negativos. Si $\lambda$ es muy grande, por ejemplo, en sus solución los coeficientes tendrán que ser muy cercanos a cero.
...
modelo_formula <- as.formula(Ozone  ~  Temp + I(Temp^2) + Wind + I(Wind^2)+
              ...
              Wind:Solar.R)
mod_sin_reg <- lm(modelo_formula, data = dat_1)
```
...
curvaturas decrecientes. No deberíamos poner mucho crédito en las predicciones
...
cuadrados regularizados. Tomamos $lambda = 3$ y  $\lambda = 13000$


```{r, message = FALSE}
library(glmnet)
dat_1 <- air %>% filter(!is.na(Ozone) & !is.na(Solar.R))
mat_ozono <- model.matrix(modelo_formula, data = dat_1)
mod_reg <- glmnet(x = mat_ozono, y = dat_1$Ozone, alpha = 0, lambda = exp(seq(-20,20, 10)))
```
...
g_lineal_reg_1 <- ggplot(air, aes(x = Solar.R, y = Ozone,  colour = Temp)) + geom_point() +
    ...
    scale_colour_gradientn(colours = rainbow(2, rev = TRUE)) +
    geom_line(data = pred_grid, aes(y = Ozone, group = interaction(Temp, Wind_cat)), size = 1) +
    labs(subtitle = "Lineal regularizado, lambda = 3") 
```
...
4. Probamos un número chico de modelos con la muestra de prueba, como estimaciones
...
set.seed(11)
seleccion_reg <- cv.glmnet(x = mat_ozono, y = dat_1$Ozone, alpha = 0, lambda = exp(seq(2,-4, -0.1)), nfolds = 10)
...
**Obs**: en este ejemplo particular, usamos validación cruzada, que funciona
...
La solución a este problema, entonces, no tiene atajos: igual que en análisis exploratorio,
