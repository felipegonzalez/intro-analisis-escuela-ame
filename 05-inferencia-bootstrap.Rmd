...
grupo (la mediana muestral), y las rectas mustran un intervalo de 90\%
para nuestra estimación de la mediana: esto quiere decir que 
los valores poblacionales tienen probabilidad aproximada de 90\% de estar
...
```{r}
quantile(medianas_muestras - valor_poblacional , c(0.025, 0.975)) %>% round
```
...
El muestreo lo hacemos con reemplazo.
...
ggplot(aes(sample = precio_miles, colour = tipo, group = tipo, alpha = 0.2)) + 
...
Y podemos calcular ahora un intervalo de confianza del 90\% calculando los percentiles de
esta distribución:

```{r}
quantile(mediana_muestras, c(0.05,  0.95)) %>% round
```
...
Hacemos bootstrap sobre toda la muestra:

```{r}
boot_perfiles <- map(1:100, function(x){
    te_boot <- te %>% sample_n(nrow(te), replace = TRUE)
    calcular_perfiles(te_boot) %>% mutate(rep = x)
}) %>% bind_rows()
```

y ahora resumimos y graficamos, esta vez de manera distinta:

```{r, fig.width = 5, fig.height = 3}
resumen_perfiles <- boot_perfiles %>% group_by(how, price) %>% 
    summarise(perfil_media = mean(perfil), ymax = quantile(perfil, 0.9), ymin = quantile(perfil, 0.1)) 
