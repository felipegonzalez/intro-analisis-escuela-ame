...
Así que tomamos el punto de vista bayesiano, pero requerimos que nuestros intervalos cumplan las
garantías frecuentistas.
...
muestra_boot(fusion, nv.vv, time, median, VV / NV)
```

```{r, fig.width = 4, fig.height = 3}
reps_boot <- map_dbl(1:2000, ~ muestra_boot(fusion, nv.vv, time, mean, VV / NV))
qplot(1:length(reps_boot), sort(reps_boot))
```

y un intervalo de 90\% sería:

```{r}
quantile(reps_boot, c(0.05, 0.95))
```
