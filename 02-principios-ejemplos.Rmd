...
Podemos calcular cuantiles adicionales:
...
O ahora que tenemos este descubrimiento en mano, podríamos calcular directamente
...
Y como los residuales tienen distribución similar en cada grupo los agrupamos para resumir:
...
Que es compacto y pone en evidencia la estructura en la relación de estas dos variables.
...
las comparaciones aditivas $(y-x)$ y multiplicativas $(y-x)$ difieren en aproximadamente por un factor multiplicativo.

## Tablas de conteos {-}

Para tablas de conteos o porcentajes, las comparaciones multiplicativas tienden a ser más útiles. 
Consideremos los siguientes datos de tomadores de te:
...
tabla_2 <- te %>% group_by(how, price) %>% tally() %>% group_by(how) %>% mutate(prop = round(100 * n / sum(n))) %>% 
  select(-n) 
tabla_2 %>% spread(how, prop, fill = 0) %>% kable(type = "html")
```

Estos datos podemos examinarlos un tiempo y llegar a conclusiones, pero no son muy efectivo en mostrar
...
Leemos esta tabla como sigue: por ejemplo, los compradores de té suelto compra té *upscale* (fino) a una
...
Aquí presentamos otro análisis de tabla de conteos. En este caso, podemos obtener perfiles de una manera
distinta: trabajando directamente con los logaritmos de los conteos.
...
Veamos cómo resultaría el análisis usando logaritmos:
```{r}
ganado <- ganado %>% 
  ungroup %>% 
...
Veamos cómo resultaría el análisis usando logaritmos, que no es muy bueno (ajustados y residuales tienen
información similar):
```{r, echo=FALSE}
ganado_nl <- ganado %>% 
  ungroup %>% 
  mutate(media = mean((count))) %>% 
  group_by(country) %>% 
  mutate(por_pais = mean((count)) - media) %>% 
  group_by(livestock.type) %>% 
  mutate(por_tipo = mean((count)) - media) %>% 
  mutate(residual = (count) - (media + por_pais + por_tipo)) %>% 
  mutate(ajustado = media + por_pais + por_tipo)
ggplot(ganado_nl, aes(x=ajustado, y=residual)) + geom_point()
```


Podemos ahora ordenar la tabla de ajustados y residuales, marcar residuales grandes, y ordenar
con alguna categoría, como hiciemos en el ejemplo del té:
