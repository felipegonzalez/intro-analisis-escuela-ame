...
La indicación que obtenemos es diferente: ahora parece ser que estados con más gasto
...
casas_tbl <- casas_ejemplo %>% group_by(calidad_gral) %>% 
    summarise(cuantiles = list(cuantil(log(precio_miles), c(0.05, 0.25, 0.5, 0.75, 0.95)))) %>% 
    unnest
...
```{r}
casas_tbl <- casas_ejemplo %>% group_by(calidad_gral) %>% 
    summarise(cuantiles = list(cuantil((precio_miles), c(0.05, 0.25, 0.5, 0.75, 0.95)))) %>% 
    unnest
