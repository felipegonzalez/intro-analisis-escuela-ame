...
menos en las colas, entonces conforme tomamos una fracción más grande de alumnos, y promedio
baja cada vez más lentamente.
...
```{r}
sat$residuales_frac <- residuals(lm(sat ~ log(frac), data = sat))
ggplot(sat, aes(x = expend, y = residuales_frac)) + geom_point() +
    geom_smooth(span = 1, size = 0)
```
