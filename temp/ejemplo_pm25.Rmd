...
Consideramos los datos de 4 trimestres, 5 estaciones y 3 años: Podemos graficar
para darnos una idea general de lo que está pasando:

```{r}
agregada %>% select(año, HORA, trimestre, estacion, percentil_99) %>% spread(estacion, percentil_99)
ggplot(agregada %>% filter(trimestre %in% c(1:12)), aes(x = HORA, y = percentil_99, colour = estacion, group = estacion)) + geom_line() +
    facet_grid(trimestre ~ año) + theme(legend.position="right") +
    scale_color_colorblind()
```

Hay varios patrones que podemos analizar en estos datos.
Podemos comenzar nuestra hacer nuestro análisis para un año dado, por ejemplo 2017 en el primer trimestre


```{r}
calc_ajuste <- function(tbl, col, reng, valor){
    tbl <- tbl %>% ungroup %>% 
        mutate(media_total = mean({{ valor }})) %>% 
        group_by({{ col }}) %>% 
        mutate(valor_columna = mean({{ valor }} - media_total)) %>% 
        group_by({{ reng }}) %>% 
        mutate(valor_renglon = mean({{ valor }}  - media_total)) %>% 
        ungroup %>% 
        mutate(residual = {{ valor }} - (media_total + valor_columna + valor_renglon ))
...
marcar_tabla <- marcar_tabla_fun(10)
...
prep_tabla <- function(ajuste, columna, renglon){
    ...
    renglones_tbl <- ajuste %>% select({{ renglon }}, efecto_r = valor_renglon) %>% unique 
...
```{r}
ajuste <- agregada %>% filter(año == 2019, trimestre == 2) %>% 
