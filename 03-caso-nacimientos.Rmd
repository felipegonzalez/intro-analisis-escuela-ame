...
library(tidyverse)
library(lubridate)
natalidad <- readRDS("./datos/natalidad/natalidad.rds") %>% 
...
```{r}
mod_1 <- loess(n ~ as.numeric(fecha), data = natalidad, span = 0.3, degree = 2)
...
```{r}
mod_anual <- loess(res_1 ~ yday(fecha), data = datos_dia, 
               degree = 0, family = "symmetric", span = 0.02)
datos_dia <- datos_dia %>%
...
```{r}
datos_dia <-  datos_dia_1 %>% 
    mutate(n_1 = n - ajuste_2 - ajuste_3 + res_3)
ggplot(datos_dia, aes(x = fecha, y = n_1)) + geom_line()
```
```{r}
mod_1 <- loess(n_1 ~ as.numeric(fecha), data = datos_dia, 
               span = 0.05, degree = 2, family = "symmetric")
datos_dia <- datos_dia %>% ungroup %>% 
    mutate(ajuste_4 = fitted(mod_1)) %>% 
    mutate(res_4 = n - ajuste_4) %>% 
    mutate(n_2 = n - ajuste_4 - ajuste_3)
ggplot(datos_dia, aes(x = fecha)) +
    geom_point(aes(y = n_1), alpha = 0.2, size = 1) +
...
mod_anual <- loess(n_2 ~ as.numeric(fecha), data = datos_dia, 
               degree = 2, family = "symmetric", span = 0.007)
datos_dia <- datos_dia %>%
...
ggplot(datos_dia, aes(x = fecha)) +
    ...
    geom_line(aes(y = ajuste_5), colour = "red") 
...
```{r}
datos_dia$dia_año <- yday(datos_dia$fecha)
ggplot(datos_dia %>% filter(año > 2000), aes(x = fecha, y = res_6, 
                                             group = factor(año))) + 
