...
- Sólo observamos una muestra de la población  o sistema de interés (a veces por costo, a veces porque a veces observaciones de interés están en futuro, por ejemplo).
- Existen variables para las cuales no tenemos información que influyen en las medidas de interés

En estos casos, el problema es que **las comparaciones que hacemos están sujetas a fluctuaciones de origen desconocido**: variación muestral, acción de variables desconocidas, por ejemplo. Cuando hacemos comparaciones, entonces, debemos de alguna
...
- En segundo lugar, y bajo ciertos supuestos, podemos considerar modelos para esas fluctuaciones que nos permitan separar señal y ruido en los datos, o entender qué tanto ruido producen esas fluctuaciones en nuestras comparaciones.

Por ejemplo:

- Si usamos una muestra de una población para generalizar a la población, queremos separar de
aspectos de nuestra muestra que generalizan a la población de otros que sólo dependen de variación muestral
- Cuando queremos juzgar si un conjunto de datos observados es consistente con un modelo o con otro conjunto de datos observados, considerando que hay variables que no controlamos que
...
- Un sistema tiene cierto comportamiento "usual" para los cuales tenemos datos históricos. El sistema presenta fluctuaciones en el tiempo.
- Observamos la última salida de nuestro sistema. Naturalmente, tiene fluctuaciones. ¿Esas fluctuaciones son consistentes con la operación usual del sistema? ¿Existe
...
Y aquí vemos que hacer **este análisis de manera tan simple no es muy
efectivo**: seguramente podemos encontrar maneras en que la nueva muestra (4) es diferente
...
para que no sepamos cuál es cuál? El juego es: 
...
¿Qué implica que la gráfica que escogamos como "más diferente" no sean los datos nuevos? ¿Qué implica que le "atinemos" a la gráfica de los datos nuevos?
...
la probabilidad de que hayamos señalado la nueva serie "sobreinterpretando" fluctuaciones en un proceso que sigue comportándose normalente es 0.05 - relativamente baja.
...
datos correctos cuando la hipótesis nula es cierta (el sistema no ha cambiado). En el caso de 20 paneles, la significancia es de 1/20 = 0.05

5. Si acertamos, y la diferencia es más notoria y fue muy fácil detectar la gráfica diferente,
esto también sugiere más evidencia en contra de la hipótesis nula. Esta dificultad es como un valor p.
...
Supongamos tenemos mediciones para tres grupos 
...
Como en el ejemplo anterior necesitamos construir o obtener una distribución de referencia (quisiéramos tomar muestras para ver qué ocurre cuando los grupos son iguales), y después comparar los datos que observamos con la referencia.

Una manera de producir comparaciones de referencia en este caso usando permutaciones.
...
**Ejercicio**: ¿dónde están los datos observados? ¿Qué tan diferentes son los
...
Cuando nos interesa una diferencia particular en los datos (¿slas de los grupos medias son distintas? ¿Sus rangos intercuartiles son distintos?, etc.), podemos
producir alguna *estadística* numérica. 

En nuestros ejemplos anteriores, nuestra estadística era una gráfica, y buscábamos identificar si la estadística para nuestra muestra era *extrema* comparado con
...
- Bajo nuestra hipótesis nula, producimos una cantidad grande de muestras permutando las etiquetas de los grupos. Esta es nuestra *distribución de referencia*
...
set.seed(118)
# produce 10000 permutaciones de las etiquetas de los grupos
...
Graficamos de dos maneras: con gráfica de cuantiles e histograma (la última es más común). Marcamos
...
probabilidad de observar una diferencia en valor absoluto tan o más extrema de lo que observamos? Podemos calcular
la respuesta como `

```{r}
1 - abs(dist_acumulada_perm(dif_obs) - dist_acumulada_perm(-dif_obs))
```
...
En este caso, lo hacemos para la diferencia de medianas:
...
es seguido por varias áreas y revistas. 
...
Dado un juego de datos, las justificaciones de las decisiones que se toman en cada paso son razonables, pero con datos distintos las decisiones podrían ser diferentes. 
...
```{r}
valor_p_fusion(fusion %>% sample_frac(0.8), 
               stat_fusion = mean, trans = identity, comp = VV - NV)
valor_p_fusion(fusion %>% sample_frac(0.8), 
               stat_fusion = median, trans = identity, comp = VV / NV)
valor_p_fusion(fusion %>% sample_frac(0.8), 
...
es exploratorio (recordemos el proceso complicado del análisis de datos de refinamiento de preguntas). En este tipo de trabajo, reportar valores p puede tener poco sentido,
...
En áreas como la sicología, existen ahora movimientos fuertes en 
favor de la replicación de estudios prometedores pero donde hay sospecha
de grados de libertad del investigador. 
