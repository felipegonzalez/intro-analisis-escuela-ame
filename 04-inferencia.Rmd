...
que dentro de cada grupo, e proceso e selección de los elementos se hace
de manera simétrica (por ejemplocada elemento tiene a misma probabiidad de ser seleccionado,
...
```{r, message = FALSE}
# ojo: esto se puede hacer más eficientemente escribiendo código
# que permute y calcule directamente la estadística de interés
...
valor_p_fusion <- function(fusion, stat_fusion = stat_fusion, 
          trans = identity, comp = VV / NV, reps = NULL, n = 10000){
    reps <- lineup(null_permute("nv.vv"), pos = 1, fusion, n = n)
    ...
    2 * min(dist_perm(dif), (1 - dist_perm(-dif)))
}
```
...
valor_p_fusion(fusion %>% sample_frac(0.95), 
               stat_fusion = mean, trans = identity, comp = VV - NV, n = 4000)
valor_p_fusion(fusion %>% sample_frac(0.95), 
               stat_fusion = median, trans = identity, comp = log(VV / NV), n = 4000)
valor_p_fusion(fusion %>% sample_frac(0.95), 
               stat_fusion = media_cuartiles, trans = identity, comp = log(VV / NV), n = 4000)
valor_p_fusion(fusion %>% sample_frac(0.95), 
               stat_fusion = median, trans = log, comp = VV - NV, n = 4000)
valor_p_fusion(fusion %>% sample_frac(0.95), 
               stat_fusion = mean, trans = log, comp = VV - NV, n = 4000)
