...
Hay otras maneras de construir distribuciones de referencia. Una manera versátil de producirlas es usando pruebas de permutaciones. 
...
- Si la persona escoge los verdaderos datos, encontramos eviencia en contra de la hipótesis nula
...
producir alguna *estadística de prueba numérica*, en lugar de una estadística gráfica.
...
- Bajo nuestra hipótesis nula, producimos una cantidad grande (por ejemplo 10 mil o más) de muestras permutando las etiquetas de los grupos. Esta es nuestra *distribución de referencia* (ya no estamos limitados a 20 replicaciones como en las pruebas gráficas).
...
```{r}
dist_perm(-dif_obs) + (1 - dist_perm(dif_obs))
```
...
g_2 <- ggplot(reps_mediana, aes(x = diferencia)) + geom_histogram(binwidth = 0.5) +
    geom_vline(xintercept = dif_obs, colour = "red") + coord_flip()
gridExtra::grid.arrange(g_1, g_2, ncol = 2) 
```
...
Lo que muestra evidencia considerable, aunque no muy fuerte, de que la instrucción verbal ayuda a reducir el tiempo de fusión de los estereogramas. Regresaremos a este ejemplo para discutir la elección de estadística usada y tratamiento de los datos.
